...
with recursive
    Hierarchy as (select employee_id, employee_name, manager_id, salary, 1 as level
                  from employees
    ...
    Subordinates as (select manager_id, employee_id as subordinate_id
                     from employees
    ...
    ManagerStats as (select s.manager_id, COUNT(*) as team_size, SUM(e.salary) as team_salary
                     from subordinates s
                              join employees e on s.subordinate_id = e.employee_id
                     group by s.manager_id),
    FinalResult as (select h.employee_id,
                           h.employee_name,
                           h.level,
                           COALESCE(ms.team_size, 0)              as team_size,
                           h.salary + COALESCE(ms.team_salary, 0) as budget
                    from Hierarchy h
                             left join
